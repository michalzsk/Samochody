...
namespace PROJEKT
{
    internal class Program
    {
        ...
        static void FilterCars()
        {
            ...
            Console.WriteLine("5. Rocznik");
            Console.Write("Wybierz kryterium (1-5): ");
            ...
            switch (filterChoice)
            {
                case 1:
                    Console.Write("Podaj typ paliwa (benzyna/diesel/elektryczne): ");
                    string fuelType = Console.ReadLine().ToLower();
                    string[] fuelTypes = { "benzyna", "diesel", "elektryczne" };
                    while (fuelType != "benzyna" && fuelType != "diesel" && fuelType != "elektryczne")
                    {
                        Console.WriteLine($"Nie istnieje {fuelType}, spróbuj ponownie");
                ...
                case 4:
                    ...
                    while (minHorsePower >= maxHorsePower)
                    {
                        Console.WriteLine($"Wprowadzono błędny przedział. Twoja minimalna moc silnika wynosi {minHorsePower}");
                        Console.WriteLine("Podaj maksymalną moc silnika (KM): ");
                        maxHorsePower = int.Parse(Console.ReadLine());
                    }
                    filteredCars = CarList.Where(car => car.CarInfo.HorsePower >= minHorsePower && car.CarInfo.HorsePower <= maxHorsePower).ToList();
                    break;
                case 5:
                    Console.Write("Podaj minimalny rocznik: ");
                    int minYear = int.Parse(Console.ReadLine());
                    Console.Write("Podaj maksymalny rocznik: ");
                    int maxYear = int.Parse(Console.ReadLine());
                    while (minYear >= maxYear)
                    {
                        Console.WriteLine($"Wprowadzono błędny przedział. Twój minimalny rocznik wynosi {minYear}");
                        Console.WriteLine("Podaj maksymalny rocznik: ");
                        maxYear = int.Parse(Console.ReadLine());
                    }
                    filteredCars = CarList.Where(car => car.CarInfo.Year >= minYear && car.CarInfo.Year <= maxYear).ToList();
                    break;
                default:
                    Console.WriteLine("Niepoprawny wybór!");
                    return;
